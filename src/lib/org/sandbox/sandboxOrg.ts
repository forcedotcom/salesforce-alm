...
export class SandboxOrg extends EventEmitter {
    ...
    constructor(private prodOrg: Org, private wait: Duration, private logger: Logger, private userSuppliedClientId?: string) {
        super();
    ...
    protected getMaxPollingRetries() {
        return this.wait ? this.wait.seconds / SandboxConstants.DEFAULT_POLL_INTERVAL.seconds : SandboxConstants.DEFAULT_MAX_RETRIES;
    }
    ...
    private async writeAuthFile(sandboxProcessObj : SandboxProcessObject, sandboxRes: SandboxUserAuthResponse) {
        ...
        if (sandboxRes.authUserName) {
            ...
            throw SfdxError.create(new SfdxErrorConfig('salesforce-alm', 'org', 'missingAuthUsername', [sandboxProcessObj.SandboxName]));
        }
    }

    private async validateSandboxCompleteAndGetAuthenticationInfo(sandboxProcessObj : SandboxProcessObject) : Promise<SandboxUserAuthResponse> {
     ...
     */
    protected async pollStatusAndAuth(sandboxProcessObj : SandboxProcessObject, retries: number, shouldPoll : boolean) : Promise<SandboxProcessObject>{
        ...
        let response: SandboxUserAuthResponse = await this.validateSandboxCompleteAndGetAuthenticationInfo(sandboxProcessObj);
        if (response) {
            try {
        ...
        if( !pollFinished ) {
            if (retries > 0){
                ...
                let polledSandboxProcessObj : SandboxProcessObject = await this.api.querySandboxProcessById(sandboxProcessObj.Id);
                return this.pollStatusAndAuth(polledSandboxProcessObj, retries - 1, shouldPoll);
            } else {
                if (shouldPoll) {
                    //timed out on retries
                    throw SfdxError.create(new SfdxErrorConfig('salesforce-alm', 'org', 'pollingTimeout', [sandboxProcessObj.Status]));
                }
                else {
