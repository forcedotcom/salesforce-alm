...
export class SandboxOrgApi {
    ...
    static readonly QUERY_SANDBOXPROCESS_FIELDS : string = "Id, Status, SandboxName, SandboxInfoId, LicenseType, CreatedDate, CopyProgress, SandboxOrganization, SourceId, Description, EndDate ";
    ...
    public async querySandboxProcessById(processId : string) : Promise<SandboxProcessObject>{
        ...
        let record : SandboxProcessObject =  await this.queryToolingApi(queryStr, 'SandboxProcessNotFoundById', 'MultiSandboxProcessFoundById', [processId], [processId]);
    ...
    public async querySandboxProcessBySandboxOrgId(sandboxOrgId : string) : Promise<SandboxProcessObject>{
        ...
        let record : SandboxProcessObject =  await this.queryToolingApi(queryStr, 'sandboxProcessNotFoundByOrgId', 'multiSandboxProcessFoundByOrgId', [sandboxOrgId], [sandboxOrgId]);
        this.logger.debug('Return from calling queryToolingApi: %s ', record);
        return record;
    }
    ...
    public async queryLatestSandboxProcessBySandboxName(sandboxNameIn : string) : Promise<SandboxProcessObject> {
        ...
        let record : SandboxProcessObject =  await this.queryToolingApi(queryStr, 'SandboxProcessNotFoundBySandboxName', 'MultiSandboxProcessNotFoundBySandboxName', [sandboxNameIn], [sandboxNameIn]);
        this.logger.debug('Return from calling queryToolingApi: %s ', record);
        return record;
    }
    ...
    private async queryLatestSandboxProcessBySandboxInfo(sandboxInfoIdIn : string) : Promise<SandboxProcessObject> {
        ...
        let record : SandboxProcessObject =  await this.queryToolingApi(queryStr, 'SandboxProcessNotFoundByInfoId', 'MultiSandboxProcessFoundByInfoId', [sandboxInfoIdIn], [sandboxInfoIdIn]);
        this.logger.debug('Return from calling queryToolingApi: %s ', record);
        return record;
    }

    private async queryToolingApi(queryStrIn: string, key0: string, key1: string, tokens0?: string[], tokens1?: string[]) : Promise<any>{
