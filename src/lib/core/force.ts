...
import { getJwtAudienceUrl } from '@salesforce/core';
...
Force.prototype.jwtAuthorize = function (oauthConfig) {
  return Promise.all([fsReadFile(oauthConfig.privateKeyFile, 'utf8'), getJwtAudienceUrl(oauthConfig)])
...
Force.prototype.getAuthorizationUrl = function (oauthConfig) {
  ...
  if (this.codeVerifier) {
    ...
    const codeChallenge = srcDevUtil.base64UrlEscape(
      crypto.createHash('sha256').update(this.codeVerifier).digest('base64')
    );
